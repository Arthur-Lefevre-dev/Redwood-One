...
| `/api/public/watch-ads` | JSON sans auth : **A-ADS** — `aads` (film : `unit_id`, `mobile_unit_id`), `aads_auth` (connexion : `unit_id` bas, optionnel `top_unit_id` / `top_enabled`) ; `WATCH_ADS_AADS_*`, `WATCH_ADS_AADS_AUTH_*`, `WATCH_ADS_AADS_AUTH_TOP_*` |
